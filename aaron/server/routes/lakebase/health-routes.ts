...
interface HealthAppKit {
  lakebase: { query: LakebaseQueryFn };
  analytics: {
    query: (
      query: string,
...
function bindAnalyticsQuery(
  analytics: HealthAppKit['analytics'],
): AnalyticsQueryFn {
  return (query, parameters) =>
    analytics.query(query, parameters).then((result) => ({
      rows: result.rows,
    }));
}

export async function setupHealthRoutes(appkit: HealthAppKit) {
  try {
    ...
    console.warn('[lakebase] Health schema setup failed:', (err as Error).message);
  }
  ...
  appkit.server.extend((app) => {
    app.post('/api/sms/inbound', async (req, res) => {
      try {
        ...
        const { reply, session } = await processSmsMessage(
          appkit.lakebase.query.bind(appkit.lakebase),
          analyticsQuery,
          parsed.data.phone,
          parsed.data.message,
        );
    ...
    app.get('/api/sms/thread/:phone', async (req, res) => {
      try {
        ...
        const sessions = await appkit.lakebase.query(
          `SELECT id, phone, status, postal_code, age, symptoms, district, state, created_at, updated_at
           FROM app.sms_sessions WHERE phone = $1`,
          [phone],
        );
        if (sessions.rows.length === 0) {
          res.json({ session: null, messages: [], recommendations: [], coverageGap: null });
          return;
        }
        const session = sessions.rows[0];
        const messages = await appkit.lakebase.query(
          `SELECT direction, body, created_at FROM app.sms_messages
           WHERE session_id = $1 ORDER BY created_at ASC`,
          [session.id],
        );
        const recommendations = await appkit.lakebase.query(
          `SELECT facility_name, facility_phone, distance_km, specialties, rank
           FROM app.facility_recommendations WHERE session_id = $1 ORDER BY rank ASC`,
          [session.id],
        );
        const gaps = await appkit.lakebase.query(
          `SELECT nearest_distance_km, gap_threshold_km, has_coverage_gap, symptoms, postal_code
           FROM app.coverage_gaps WHERE session_id = $1`,
          [session.id],
        );
        res.json({
          session,
    ...
    app.get('/api/sms/stats', async (_req, res) => {
      try {
        ...
        const recentGaps = await appkit.lakebase.query(`
          ...
          LIMIT 20
        `);
        res.json({ stats: stats.rows[0], recentGaps: recentGaps.rows });
      } catch (err) {
