...
//#region server/agents/health-helper.ts
...
function createHealthTools(deps) {
	return {
		resolve_location: tool({
			description: "Resolve an Indian user location from a 6-digit pincode OR free-text descriptors (town / post office name, division, district, state). Pass `pincode` when the user gave one; otherwise pass `descriptors` (and optionally `district`/`state`) with whatever the user said about where they are. Returns a best-first list of candidate locations, each with lat/lon and a geoConfidence (0..1), plus `needsDisambiguation`: when true, ASK the user a clarifying question (e.g. which district, nearest town) and call this tool again with the added detail before proceeding. When false, the top candidate is a confident match. Read-only.",
			schema: z.object({
				pincode: z.string().optional().describe("6-digit Indian pincode, if the user provided one"),
				descriptors: z.string().optional().describe("Free text describing the location: town, post office, landmark, area"),
		...
		match_facilities: tool({
			description: "Find the nearest health facilities able to treat the user, given their resolved location (lat/lon from resolve_location) and their symptoms. Symptoms are mapped to a medical specialty internally. Pass `geoConfidence` from the chosen location so the result's facilityConfidence reflects location certainty. Returns ranked facilities (name, phone, distanceKm, specialties, facilityConfidence 0..1), the derived specialty, a symptomConfidence (0..1), and `hasCoverageGap` (true when the nearest facility is >50 km away). Read-only.",
			schema: z.object({
			...
			execute: async (args) => {
				...
				return {
					...
					nearestDistanceKm: match.nearestDistanceKm == null ? null : Number(match.nearestDistanceKm.toFixed(1)),
					facilities: match.facilities.map(facilitySummary)
				};
			}
		}),
		build_intake_bundle: tool({
			description: "Assemble and persist the final intake bundle once a location is confidently chosen and facilities have been matched. Provide the symptom summary, the raw inputs used, the candidates considered, the chosen location, and the matched facilities (use the objects returned by resolve_location and match_facilities). Persists to Lakebase and returns the stored bundle (with a bundleId) including geoConfidence and facilityConfidence. Call this LAST, exactly once, before giving the user the summary.",
			schema: z.object({
				...
				candidatesConsidered: z.array(z.object({
			...
			execute: async (args) => {
				const toCandidate = (c) => ({
					...
					lon: c.lon,
					matchedRows: 1,
					matchLevel: c.matchLevel ?? "state",
					geoConfidence: c.geoConfidence
				});
				const facilities = args.facilities.map((f) => ({
					name: f.name,
					address_city: null,
				...
				try {
					...
					console.warn("[intake] Failed to persist bundle:", err.message);
				}
				return {
					bundleId,
					bundle
...
* agent.
*/
function createHealthHelperAgent(deps) {
	return createAgent({
		instructions: "You execute health-intake actions: resolve a location, match facilities, and build the intake bundle. Use the tools precisely and return their structured results.",
		tools: createHealthTools(deps)
	});
}

//#endregion
export { createHealthHelperAgent, createHealthTools };